...
**CI.** Секреты в репо не обязательны: тесты на H2, сборка без подстановки. Если понадобится передавать в воркфлоу — те же имена в Repository secrets: `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `JWT_SECRET`.
